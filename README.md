...
Open the App Preview panel in Emergent (top‑right).
...
Sign up with email/password (or tap Continue with Google)
Land on the Home dashboard → tap "+1,000 steps" or "Log manually" to fill in steps, water
Workouts tab → tap + → build a workout (name + exercises with sets) → Save
Nutrition tab → tap + → log a meal with calories & macros
Coach tab → tap a suggestion or type a question; Claude answers using your stats
Progress tab → see your weekly bars
...
The AI Coach is positioned as a premium hook — usage can later be metered behind a "Stride Pro" subscription (e.g., free 10 messages/day → unlimited with Stripe). The dashboard, logger and progress charts remain free, driving daily engagement.
